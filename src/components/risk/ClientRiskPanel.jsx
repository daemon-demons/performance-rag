      ...
      {spofAlerts.length > 0 && (
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-4">
          <div className="flex items-start gap-2">
            <AlertTriangle className="mt-0.5 shrink-0 text-rag-amber" size={18} />
            <div>
              <h3 className="font-display font-semibold text-amber-900">
                SPOF / Client Risk Alerts
              </h3>
              <p className="mt-1 text-sm text-amber-800">
                Clients with fewer than 2 people at Max_V93k ≥ 8 or Lab_Score ≥ 8:
              </p>
              <ul className="mt-2 space-y-1 text-sm text-amber-900">
                {spofAlerts.map((c) => (
                  <li key={c.client}>
                    ...
                    {' — '}
                    {c.highSkillCount} high-skill engineer
                    {c.highSkillCount === 1 ? '' : 's'}
                  </li>
                ))}
              </ul>
            </div>
        ...
        <table className="min-w-full divide-y divide-slate-200 text-sm">
          <thead className="bg-slate-50 text-xs tracking-wide text-slate-500 uppercase">
            <tr>
              ...
              <th className="px-3 py-3 text-left font-semibold">Headcount</th>
              <th className="px-3 py-3 text-left font-semibold">High Skill</th>
              <th className="px-3 py-3 text-left font-semibold">Health</th>
              <th className="px-3 py-3 text-left font-semibold">SPOF</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {clients.map((c) => (
              <tr key={c.client} className="hover:bg-slate-50">
                ...
                <td className="px-3 py-2.5 text-slate-700">{c.total}</td>
                ...
                <td className="px-3 py-2.5">
                  <div className="flex flex-wrap gap-1.5 text-xs">
                    ...
                    <span className="text-slate-500">D {c.health.departedPct}%</span>
                  </div>
                </td>
                <td className="px-3 py-2.5">
                  {c.isSpof ? (
